...
namespace AccordVideoPlus
{
    ...
    /// </remarks>
    /// 
    public class AccordAVIFileVideoSourcePlus : IVideoSource
    {
        // video file name
        ...
        public void UnPause() => IsPausing = false;

        public int Position
        {
            get => aviReader?.Position ?? 0;
            set
            {
                if (aviReader != null)
                    aviReader.Position = value;
            }
        }

        public double SourceFrameRate => aviReader?.FrameRate ?? -1;

        public double PositionSec
        {
            get => SourceFrameRate == 0 ? 0 : Position / SourceFrameRate;
            set
            {
                Position = (int)(value * SourceFrameRate);
            }
        }

        public int Length => aviReader?.Length ?? -1;

        public double LengthSec => SourceFrameRate == 0 ? 0 : Length / SourceFrameRate;

        /// <summary>
        /// Initializes a new instance of the <see cref="AccordAVIFileVideoSourcePlus"/> class.
        /// </summary>
        /// 
        public AccordAVIFileVideoSourcePlus() { }
        ...
        /// 
        public AccordAVIFileVideoSourcePlus(string source)
        {
            this.source = source;
        }
